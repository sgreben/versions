...
func parseAction(name, text string) (*ParserState, error) {
	p, err := Parser(name, fmt.Sprintf("%s%s%s", leftDelim, text, rightDelim))
...
func (p *ParserState) consumeText() string {
...
func (p *ParserState) backup() {
	p.pos -= p.width
}

func (p *ParserState) parseText(cur *ListNode) error {
	for {
...
func (p *ParserState) parseLeftDelim(cur *ListNode) error {
	p.pos += len(leftDelim)
	p.consumeText()
	newNode := newList()
	cur.append(newNode)
...
func (p *ParserState) parseInsideAction(cur *ListNode) error {
...
func (p *ParserState) parseRightDelim(_ *ListNode) error {
...
func (p *ParserState) parseIdentifier(cur *ListNode) error {
	var r rune
	for {
		r = p.next()
		if isTerminator(r) {
...
func (p *ParserState) parseRecursive(cur *ListNode) error {
	p.pos += len("..")
	p.consumeText()
...
func (p *ParserState) parseNumber(cur *ListNode) error {
	r := p.peek()
	if r == '+' || r == '-' {
		p.next()
	}
...
func (p *ParserState) parseArray(cur *ListNode) error {
Loop:
	for {
		switch p.next() {
		case eof, '\n':
...
func (p *ParserState) parseFilter(cur *ListNode) error {
	p.pos += len("[?(")
	p.consumeText()
	begin := false
	end := false
...
func (p *ParserState) parseQuote(cur *ListNode, end rune) error {
Loop:
	for {
		switch p.next() {
		case eof, '\n':
...
func (p *ParserState) parseField(cur *ListNode) error {
	p.consumeText()
	for p.advance() {
	}
	value := p.consumeText()
...
func (p *ParserState) advance() bool {
	r := p.next()
	if r == '\\' {
		p.next()
